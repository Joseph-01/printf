 ...
 * @conversions: List a of arguments
...
{
int length = 0, i;
char *str = va_arg(conversions, char *);
UNUSED(buffer);
UNUSED(flags);
UNUSED(width);
UNUSED(precision);
 ...
 * Return: Number of chars printed
 */
int print_int(va_list conversions, char buffer[],
int flags, int width, int precision, int size)
{
int i = BUFF_SIZE - 2;
int is_negative = 0;
long int n = va_arg(conversions, long int);
...
UNUSED(precision);
UNUSED(size);
n = va_arg(conversions, unsigned int);
m = 2147483648; /* (2 ^ 31) */
a[0] = n / m;
for (i = 1; i < 32; i++)
{
